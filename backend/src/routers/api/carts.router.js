...
router.post('/carts', passport.authenticate('jwt', { session: false }), authMiddleware(['premium','user','admin']), async (req, res, next) => {
    try {
...
router.get('/carts/:cid', passport.authenticate('jwt', { session: false }), authMiddleware('user', 'admin','premium'), async (req, res, next) => {
    try {
